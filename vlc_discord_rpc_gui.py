...
class DiscordManager(threading.Thread):
    ...
    def set_state(self, new_state, message):
        ...
        health_status = "HEALTHY" if new_state == "CONNECTED" else ("DEGRADED" if new_state in ("CONNECTING", "RECONNECTING") else "OFFLINE")
        if getattr(self.backend_ref, "diagnostics", None):
            self.backend_ref.diagnostics.set_state("discord", health_status, message, is_success=(new_state=="CONNECTED"))
        if message:
...
class RPCBackend:
    ...
    def log(self, msg):
        ...
        if getattr(self, "diagnostics", None):
            self.diagnostics.log_event(msg, component="system")
        if self.window:
            try:
  ...
  if (code) {
    ...
    def fetch_anilist_username(self):
        ...
        try:
            ...
            if getattr(self, "diagnostics", None):
                import sys
                self.diagnostics.report_error("anilist", "IdentityAPIError", str(e), exc_info=sys.exc_info())
            pass
        ...
        while not self.state_data["exit_flag"]:
            ...
            try:
                ...
                if r.status_code == 200:
                    ...
                    if getattr(self, "diagnostics", None):
                        self.diagnostics.set_state("vlc", "HEALTHY", "VLC connected", is_success=True)
...
class WebApi:
    ...
    def get_diagnostics(self):
        if getattr(self._backend, "diagnostics", None):
            return self._backend.diagnostics.get_state()
        return {}

    def run_diagnostics(self):
        if getattr(self._backend, "diagnostics", None):
            return self._backend.diagnostics.run_self_test()
        return {"status": "error", "message": "DiagnosticsManager not initialized"}

    def export_diagnostics(self):
        if getattr(self._backend, "diagnostics", None):
            return self._backend.diagnostics.export_diagnostics()
        return None

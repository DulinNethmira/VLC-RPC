...
def main():
    ...
    release_title = "✨ v5.3.4 - Auto-Updater Hotfix"
    release_notes = """### 🚀 What's New in v5.3.4!

This is a hotfix for the in-app updater and UI.

#### 🔧 Fixes & Tweaks
- **Fixed Update Loop:** The previous `v5.3.3` release had a hardcoded version string that made the app think it was still `v5.3.2`, causing an endless update loop. This version correctly identifies itself and breaks the loop.
- **Fixed Markdown in Update Modal:** The update changelog now correctly renders rich formatting (bold, italics, lists, and headers) instead of raw markdown symbols inside the app.

*(If you were stuck in the update loop from `v5.3.3`, updating to this version will finally clear the prompt!)* 🎉"""
